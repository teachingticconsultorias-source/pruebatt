...
const FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    // Seleccionada: se tiñe hacia el verde azulado de SciVerse y se le sube
    // el borde. Deliberadamente no es un color fluorescente: tiene que
    // destacar sobre el modelo, no competir con él.
    float elegida = step(2.5, vEstado);
    color = mix(color, mix(color, colorSeleccion, 0.55) + borde * 0.30, elegida);
...
export function crearVisor({
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      ...
      colorSeleccion: { value: new THREE.Vector3(0.09, 0.46, 0.44) },
  ...
  function onMove(e) {
    ...
    if (pinchaPrevia > 0 && distancia > 0) radio *= pinchaPrevia / distancia;
  ...
  return {
    ...
    /**
     * Encuadra una estructura concreta.
     *
     * La distancia sale del tamaño de SU caja y del campo de visión, no de un
     * número fijo: un nervio nasociliar y un fémur no se ven bien desde la
     * misma distancia, y con un zoom único uno queda diminuto y el otro
     * recortado. Se conserva el ángulo actual para no desorientar.
     */
    enfocar(indice) {
      const p = partes[indice];
      if (!p) return;
      const min = punto.fromArray(p.bounds[0]).clone();
      const max = punto.fromArray(p.bounds[1]).clone();

      objetivo.copy(min).add(max).multiplyScalar(0.5);

      const tamano = max.sub(min);
      const mayor = Math.max(tamano.x, tamano.y, tamano.z, diagonal * 0.004);
      const fov = THREE.MathUtils.degToRad(camara.fov);
      // Distancia a la que la estructura ocupa una parte cómoda del encuadre.
      // El 2,1 deja aire alrededor: pegarse hasta el borde impide ver con qué
      // se relaciona, que es medio motivo para mirarla.
      const distancia = (mayor * 2.1) / (2 * Math.tan(fov / 2));
      radio = THREE.MathUtils.clamp(distancia, RADIO_MIN, RADIO_MAX);
