...
import { coloresDe } from "../export/marca.js";
import { construirPatches, rellenarPlantilla } from "../export/plantilla.js";
import { TextRun } from "docx";
import { sessionChildren, sessionSections, tablaRubrica } from "./plantillas/sesion.js";
...
export function buildDocument(type, options = {}) {
  aplicarMarca(coloresDe(options.marca, { puedePlantilla: options.puedePlantilla }));
...
function bloquesPorMarcador(type, { resource: r = {}, form = {}, profile = {} }) {
  if (type === "session") {
    const hijos = sessionChildren({ form, resource: r, profile });
    // Se corta por los encabezados de sección, que es lo que la maqueta ya
    // separa: así el marcador `{{criterios}}` recibe exactamente la tabla de
    // criterios y no media sesión.
    return trocearPorSeccion(hijos, {
      datos_generales: ["DATOS GENERALES"],
      propositos: ["PROPÓSITOS DE APRENDIZAJE"],
      desempenos: ["DESEMPEÑOS PRECISADOS"],
      criterios: ["CRITERIOS DE EVALUACIÓN"],
      enfoques: ["ENFOQUES TRANSVERSALES"],
      secuencia: ["PREPARACIÓN DOCENTE", "MATERIALES", "SECUENCIA DIDÁCTICA"],
      dua: ["ORIENTACIONES DUA", "INSTRUMENTO SUGERIDO", "REFLEXIONES DEL DOCENTE"],
      anexos: ["ANEXOS PARA LA CLASE", "ANEXO 1", "ANEXO 2", "ANEXO 3", "ANEXO 4"],
    });
  }
  // Para el resto de documentos no hay un troceado natural: todo el cuerpo va
  // al marcador `{{secuencia}}`, que es el que la plantilla base reserva para
...
/**
 * Reparte los hijos de la maqueta entre marcadores.
 *
 * Se recorre una vez y se va cambiando de cubo al encontrar un encabezado de
 * sección. Lo que caiga antes del primero —título y subtítulo— se descarta:
 * en la plantilla del colegio esa parte ya la pone el colegio.
 */
function trocearPorSeccion(hijos, mapa) {
  const cubos = Object.fromEntries(Object.keys(mapa).map(k => [k, []]));
  const deTexto = new Map();
  for (const [marcador, titulos] of Object.entries(mapa)) {
    for (const titulo of titulos) deTexto.set(titulo, marcador);
  }

  let actual = null;
  for (const hijo of hijos) {
    const titulo = textoDeEncabezado(hijo);
    if (titulo) {
      const encontrado = [...deTexto.entries()].find(([clave]) => titulo.startsWith(clave));
      if (encontrado) { actual = encontrado[1]; continue; }
    }
    if (actual) cubos[actual].push(hijo);
  }
  return cubos;
}

/** El texto de un párrafo de `docx`, si lo tiene. Sin tocar sus interioridades. */
function textoDeEncabezado(hijo) {
  const runs = hijo?.root?.filter?.(n => n?.rootKey === "w:r") || [];
  const texto = runs.map(r => (r.root || []).map(x => x?.root?.text || "").join("")).join("").trim();
  return texto || null;
}
...
export async function downloadResource(type, resource, form = {}, profile = {}) {
  const marca = await marcaParaExportar();
  ...
  await download(buildDocument(type, { resource, form, profile, marca }), nombre);
}

export async function downloadCompleteClass(options) {
  const marca = await marcaParaExportar();
  await download(buildCompleteClass({ ...options, marca }), filename("Clase completa", options.session.result.titulo));
}
