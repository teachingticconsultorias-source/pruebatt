...
function questions(items, start = 0) {
  return list(items).flatMap((q, i) => {
    ...
    return [...block("Lectura", q.textoLectura),
      ...
      // «A. Verdadero / B. Falso» las imprimía dos veces: no se veía en la
      // ficha de trabajo, donde esas preguntas llegan sin `opciones`, pero el
      // esquema del cuestionario las exige siempre y ahí salía duplicado.
      ...(q.tipo === "verdadero_falso" ? [] : list(q.opciones).map((o, j) =>
        p(`${String.fromCharCode(65 + j)}. ${clean(o)}`, { size: TAMANO.cuerpo, keepNext: true, spacing: { after: 40 } }))),
      ...(q.tipo === "verdadero_falso" ? [p("☐ Verdadero     ☐ Falso", { size: TAMANO.cuerpo })] : answerSpace(q.tipo === "respuesta_larga" || q.nivel === "critico" ? 4 : 3))];
  });
}
function worksheetSections({ form = {}, resource: r = {}, type = "worksheet" }) {
  let number = 0;
  // El cuestionario comparte esta maqueta —datos del estudiante, instrucciones
  // y preguntas numeradas— pero no es una ficha de trabajo y no puede
  // titularse como tal: se descargaba encabezado «FICHA DE TRABAJO».
  const esCuestionario = type === "questionnaire";
  const rotulo = esCuestionario ? "CUESTIONARIO" : "FICHA DE TRABAJO";
  const meta = { tipo: esCuestionario ? "Cuestionario" : "Ficha de trabajo", area: form.area };
  const sections = [];
  let children = [title(rotulo), subtitle(r.titulo || form.tema || ""),
...
export function buildSections(type, options = {}) {
  ...
  if (type === "worksheet" || type === "questionnaire") return worksheetSections({ ...options, type });
...
/* --------------------------------------------------------------------------
   ...
   produce la maqueta de SciVerse, colocados dentro del .docx del colegio. Una
 ...
 * azul de SciVerse y se incrustaban tal cual en el membrete del colegio.
 */
...
/* --------------------------------------------------------------------------
   ...
   y todo sale en el formato de SciVerse, que es exactamente lo que deben hacer
...
export async function downloadResource(type, resource, form = {}, profile = {}) {
  ...
  // colegio vacíos; sale mejor con la maqueta de SciVerse y su marca. Ver
