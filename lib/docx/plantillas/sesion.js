...
/* ==========================================================================
   ...
   ========================================================================== */
/**
 * La sesión troceada por marcador, para las plantillas propias del colegio.
 *
 * POR QUÉ EXISTE ESTA FUNCIÓN
 * ---------------------------
 * La primera versión del modo «plantilla propia» partía `sessionChildren()`
 * leyendo el texto de los párrafos ya construidos, para adivinar dónde
 * empezaba cada sección. No funcionó: los objetos de `docx` no exponen su
 * texto de esa manera, la detección devolvía null para los 95 hijos, los ocho
 * cubos salían vacíos y los ocho marcadores de bloque se quedaban literales en
 * el documento de la docente.
 *
 * La lección es que la maqueta ya SABE dónde está cada sección: no hay que
 * deducirlo del resultado. Aquí se declara una vez y `sessionChildren()` se
 * compone de esto, así que no pueden separarse.
 *
 * @returns {Record<string, Array>} marcador → hijos de `docx`
 */
export function sessionBloques({ form = {}, resource: r = {}, profile = {}, rubrica = null }) {
  ...
  return {
    datos_generales: [tablaEtiquetas(filasDatos(form, profile), widths(ANCHOS.datosGenerales))],

    propositos: proposito.length
    ...
    enfoques: enfoques.length
      ? [tablaDatos(["Enfoque", "Valor", "Actitud observable"], enfoques, widths(ANCHOS.enfoques))] : [],

    // Estos dos cubos traen DENTRO sus propios encabezados de sección, porque
    // agrupan varias: la secuencia se lleva preparación y materiales, y el DUA
    // se lleva instrumento y reflexiones. En la plantilla del colegio son parte
    // de «cómo se da la clase» y «qué queda después»; separarlas obligaría a
    // declarar cuatro marcadores más que casi nadie usaría.
    secuencia: [
    ...
    anexos: anexosDe(r, rubrica),
  };
}

export function sessionChildren(opciones = {}) {
  const { form = {}, resource: r = {} } = opciones;
  const b = sessionBloques(opciones);
  const conSeccion = (titulo, hijos) => (hijos.length ? [seccion(titulo), ...hijos] : []);

  return [
    title("SESIÓN DE APRENDIZAJE"),
    subtitle(r.titulo || form.tema || ""),
    ...conSeccion("DATOS GENERALES", b.datos_generales),
    ...conSeccion("PROPÓSITOS DE APRENDIZAJE", b.propositos),
    ...conSeccion("DESEMPEÑOS PRECISADOS", b.desempenos),
    ...conSeccion("CRITERIOS DE EVALUACIÓN", b.criterios),
    ...conSeccion("ENFOQUES TRANSVERSALES", b.enfoques),
    // Estos dos ya vienen con sus encabezados dentro.
    ...b.secuencia,
    ...b.dua,
    ...b.anexos,
  ];
