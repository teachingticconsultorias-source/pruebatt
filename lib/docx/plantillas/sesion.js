...
function anexosDe(r = {}, rubrica = null) {
  ...
  anexos.forEach((anexo, i) => {
    ...
    if (hay(anexo.contenido)) salida.push(p(texto(anexo.contenido), { size: TAMANO.cuerpo }));
    if (hay(anexo.instrucciones)) salida.push(destacado(`**Instrucciones:** ${texto(anexo.instrucciones)}`));
    // Espacio real para responder en la guía de trabajo, con líneas suaves.
    if (/gu[íi]a|ficha|actividad/i.test(nombre)) {
      salida.push(p("Nombres de los integrantes del equipo:", { size: TAMANO.cuerpo, color: COLOR.navy, bold: true, keepNext: true }),
        ...answerSpace(2));
    }
  });
