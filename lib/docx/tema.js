...
const PALETA_SCIVERSE = {
   ...
   *
   * Es blanco en la maqueta de SciVerse y NADA en la neutra: un blanco explícito
...
/* --------------------------------------------------------------------------
   ...
   tablas con el navy de SciVerse dentro de la plantilla de otra institución es
...
/* --------------------------------------------------------------------------
   ...
   valores, y cada sitio que se olvidara quedaría con el color de SciVerse dentro
   ...
   aplica la marca, se arma el .docx y se vuelve a SciVerse.
   -------------------------------------------------------------------------- */
let activa = { ...PALETA_SCIVERSE };

/**
 * Fija la paleta del documento que se va a construir.
 *
 *   `null`                    la de SciVerse. Es el respaldo de todo.
 *   `{ neutra: true }`        sin identidad: el diseño lo pone la plantilla.
 *   `{ primario, acento }`    los colores del colegio sobre la maqueta.
 *
 * Con colores inválidos vuelve a SciVerse: un color roto no se «arregla» a un
 * gris cualquiera, porque al menos se sabe que el de SciVerse contrasta.
 */
export function aplicarMarca(colores) {
  ...
  activa = { ...PALETA_SCIVERSE, rubrica: { ...PALETA_SCIVERSE.rubrica } };
...
export function restablecerMarca() {
  activa = { ...PALETA_SCIVERSE };
}

/** Los valores de SciVerse, para quien necesite el original pase lo que pase. */
export const COLOR_SCIVERSE = Object.freeze({ ...PALETA_SCIVERSE });
 ...
 * que congelaba los valores de SciVerse al importar.
 */
...
function tintaSobre(fill) {
  return fill === PALETA_SCIVERSE.rubrica.ad ? activa.blanco : activa.texto;
}
