...
//
// FALLA HACIA EL FORMATO ESTÁNDAR
// -----------------
// Cualquier problema —sin sesión, sin fila, bucket caído, fichero borrado—
// devuelve la marca de SciVerse. Una docente que no puede cargar su logo debe
// seguir descargando su documento, no quedarse sin él.

import { supabase } from "../../supabaseClient.js";
import { MARCA_SCIVERSE, modoEfectivo, normalizarMarca } from "./marca.js";
import { pareceDocx } from "./plantilla.js";
...
export async function marcaVigente() {
  const user = await usuario();
  if (!user) return { ...MARCA_SCIVERSE, modoEfectivo: "estandar" };
  ...
  if (efectivo === "plantilla") {
    const bytes = await descargarArchivo(marca.plantillaPath);
    // Dos motivos para no tener plantilla utilizable, y la misma caída:
    //
    //   · el fichero se borró desde el panel de Storage;
    //   · lo que hay ahí NO es un .docx. `plantilla_path` es texto en una fila
    //     que el docente puede escribir, así que puede apuntar a cualquier
    //     cosa; intentar parchearla reventaría a mitad de la descarga.
    resuelta.plantillaBytes = pareceDocx(bytes) ? bytes : null;
    if (!resuelta.plantillaBytes) {
      resuelta.modoEfectivo = modoEfectivo({ ...marca, plantillaPath: null }, { puedePlantilla: puede });
    }
  }
